...
var fetchQuestions = async () => {
  try {
    const data = await fetch(`https://api.jsonbin.io/v3/b/635137ac65b57a31e69c9119`, {
      headers: {
...
async function run() {
  try {
    ...
    allQuestions.push(...fetchRes.record);
...
console.log('helooo');
document.querySelectorAll('.group-btn button')[0].click();
document
  .querySelectorAll(
    '#__next > section > main > div > div > div.create-test.v-layout-center.my-8 > div > div > div.test-view__right > div.relative.test-view__exam-detail > div.flex.items-center.justify-end.-mt-8',
  )[0]
